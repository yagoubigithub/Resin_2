...
$(document).ready(function () {
    ...
    $.post('../getVar', {vari: "user_id"},
            function (data) {
                vari = $.parseJSON(data);
                

                $.post('../fetchPublication', {user_id: vari.user_id},
                        function (data) {
...
<h6 class="user_name_pub"> ' + obj[i].firstname + ' ' + obj[i].lastname + ' </h6> </div></div><br> <div class="row"> <div class="col">\n\
 <p>' + obj[i].nom_article +'  ' +obj[i].id+'</p><strong><p>Prix : <mark>' + obj[i].prix + ' DA</mark></p></strong>' + '<p>Tel : ' + obj[i].tel + '</p><button type="button" class=\n\
...
</div> </div><div class="row">   <div class="col"><button type="button" onclick="sendLike(' + obj[i].id + ',' + vari.vari + ')" class="btn" id="' + obj[i].id + '"><i class="fa fa-thumbs-up i'+obj[i].ifLikeit+'"></i> Like</button>\n\
